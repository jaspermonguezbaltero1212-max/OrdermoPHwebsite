...
async function renderAreaMenu() {
    ...
    function getItemImg(name, merchant) {
        ...
        // Fallback: uploaded items without a keyword match get a generic food image
        return baseImgPath + itemImages.filipinoFood + imgParams;
    }
    grid.innerHTML = combinedMenu.map((entry, index) => {
        ...
        return `
            <div class="m-card" data-item="${entry.item}" data-price="${entry.price}" data-merchant-id="${entry.merchantId || ''}">
...
function updateAuthUI(user) {
    ...
    if (user || merchantProfile || riderProfile) {
        const name = merchantProfile?.business_name || riderProfile?.full_name || user?.displayName || user?.phoneNumber || (user?.email ? user.email.split('@')[0] : "User");
        ...
        dropdownMenu.innerHTML = `
            ...
            <hr class="dropdown-divider">
            <a href="merchant-dashboard.html" class="dropdown-item">Merchant Login</a>
            <a href="rider-dashboard.html" class="dropdown-item">Rider Login</a>
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                if (formId === 'merchantForm') {
                    ...
                    data.userId = userCred.user.uid;
                    // Normalize email to match Firebase Auth (lowercase)
                    data.email = (userCred.user.email || email).toLowerCase().trim();

                    // Strip password fields before saving to Firestore
                    delete data.password;
                    delete data.confirm_password;

                    // Auto-approve: set status so dashboard shows Approved badge
                    data.status = 'approved';
                    data.approvedAt = new Date().toISOString();

                    // Firestore writes — redirect regardless of outcome (auto-heal in dashboard)
                    try {
                        const merchantId = generateReadableId('subjects');
                        await setDoc(doc(db, 'merchant_applications', merchantId), data);
                        await setDoc(doc(db, 'merchants', merchantId), data);
                    } catch (fsErr) {
                        console.warn('[Registration] Firestore write failed (dashboard will auto-create):', fsErr);
                    }

                    const merchantData = { ...data };
                    delete merchantData.timestamp;
                    localStorage.setItem('ordermo_merchant_profile', JSON.stringify(merchantData));
                    window.location.href = 'merchant-dashboard.html';
                } else if (formId === 'riderForm') {
                    const email = data.email;
                    const password = data.password;
                    const confirm = data.confirm_password;
                    ...
                    // Firestore writes — redirect regardless of outcome (auto-heal in dashboard)
                    try {
                        const riderId = generateReadableId('animals');
                        await setDoc(doc(db, 'rider_applications', riderId), data);
                        await setDoc(doc(db, 'riders', riderId), data);
                    } catch (fsErr) {
                        console.warn('[Registration] Firestore write failed (dashboard will auto-create):', fsErr);
                    }

                    const riderData = { ...data };
                    delete riderData.timestamp;
                    localStorage.setItem('ordermo_rider_profile', JSON.stringify(riderData));
                    window.location.href = 'rider-dashboard.html';
                } else {
                    const cat = collectionName === 'merchant_applications' ? 'subjects' : 'animals';
                    await setDoc(doc(db, collectionName, generateReadableId(cat)), data);
                    if (onSuccess) {
                        onSuccess(data);
                    } else {
                        alert(successMsg);
...
function initCheckoutPage() {
    ...
    checkoutForm.addEventListener('submit', async (event) => {
        ...
        const order = {
            ...
            items: cart,
            merchantIds: [...new Set(cart.map(item => item.merchantId).filter(id => id))],
            merchantPickup: firstItem.merchant || '',
            merchantPickupArea: firstItem.area || '',
            deliveryAddress,
...
function initConfirmationPage() {
    ...
    const statusSteps = [
        { key: 'Pending merchant', label: 'Order Placed', icon: '📋' },
        { key: 'Waiting for rider', label: 'Merchant Accepted', icon: '👨‍🍳' },
    ...
    const renderTimeline = (currentStatus) => {
        ...
        return statusSteps.map((step, i) => {
            const isComplete = i <= idx && idx !== -1;
            const isCurrent = i === idx && currentStatus !== 'Delivered' && currentStatus !== 'Rejected';
            const isRejected = currentStatus === 'Rejected';
            return `
                <div class="tracking-step ${isComplete ? 'complete' : ''} ${isCurrent ? 'current' : ''}">
                    <div class="tracking-step-icon ${isComplete ? 'complete-icon' : ''} ${isRejected && i === 0 ? 'rejected-icon' : ''}">
                        ${isRejected && i === 0 ? '❌' : step.icon}
                    </div>
                    <div class="tracking-step-body">
                        <strong>${step.label}</strong>
                        ${isCurrent ? '<span class="tracking-pulse">In progress...</span>' : ''}
                        ${isRejected && i === 0 ? '<span class="tracking-rejected">Order was declined by the merchant</span>' : ''}
                    </div>
                </div>
            `;
        }).join('');
    ...
    confirmation.innerHTML = `
        <p class="status-pill" id="orderStatusPill">${order.status}</p>
        <h1>Order confirmed</h1>
        <p class="confirmation-copy" id="orderStatusText">
            ${order.status === 'Pending merchant' ? 'Your order has been sent to the merchant for review.' :
              order.status === 'Rejected' ? 'The merchant has declined your order.' :
              'Your order is on the way!'}
        </p>
        <div class="tracking-timeline" id="orderTimeline">
            ${renderTimeline(order.status)}
        </div>
        <div id="customerTrackingSection" class="tracking-section" style="display:none;">
    ...
    if (order.orderId && !order.orderId.startsWith('LOCAL-')) {
        ...
        let customerMap = null, riderMarker = null, destMarkerCustomer = null;
        ...
        onSnapshot(orderDocRef, (docSnap) => {
            ...
            if (timeline) timeline.innerHTML = renderTimeline(data.status);
            if (text) {
                const msgs = {
                    'Pending merchant': 'We\'ve sent your order to the merchant. Please wait while they review it.',
                    'Waiting for rider': 'The merchant is preparing your order. We\'re finding a rider for you!',
                    'Out for delivery': 'Your rider is on the way with your order! Sit tight and enjoy the wait.',
                    'Delivered': 'Your order has arrived! We hope you enjoy every bite. Thank you for choosing OrdermoPH!',
                    'Rejected': 'We\'re sorry, the merchant was unable to accept your order this time.'
                };
                text.textContent = msgs[data.status] || 'Your order is being processed.';
            }

            // Show live map when out for delivery
            const trackSection = document.getElementById('customerTrackingSection');
            const trackMap = document.getElementById('customerTrackingMap');
            if (data.status === 'Out for delivery' && trackSection && trackMap) {
                trackSection.style.display = 'block';
                if (!customerMap) {
                    customerMap = L.map(trackMap).setView([14.5, 121], 13);
                    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap' }).addTo(customerMap);
                    // Geocode merchant pickup location
                    if (data.merchantPickupArea) {
                        fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(data.merchantPickupArea + ' ' + (data.merchantPickup || ''))}&limit=1`)
                            .then(r => r.json()).then(geo => {
                                if (geo.length > 0) {
                                    L.marker([parseFloat(geo[0].lat), parseFloat(geo[0].lon)], {
                                        icon: L.divIcon({ html: '🏪', iconSize: [24,24], className: '' })
                                    }).addTo(customerMap).bindPopup('Merchant: ' + (data.merchantPickup || 'Store'));
                                }
                            }).catch(() => {});
                    }
                    // Geocode delivery address for destination marker
                    ...
                    fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(addrStr)}&limit=1`)
                        .then(r => r.json()).then(geoData => {
                            if (geoData.length > 0) {
                                const dl = parseFloat(geoData[0].lat), dn = parseFloat(geoData[0].lon);
                                L.marker([dl, dn], { icon: L.divIcon({ html: '📍', iconSize: [24,24], className: '' }) }).addTo(customerMap).bindPopup('Your Address');
                                customerMap.setView([dl, dn], 14);
                            }
                        }).catch(() => {});
                }
                // Update rider marker position
                if (data.riderLat && data.riderLng) {
                    if (riderMarker) riderMarker.setLatLng([data.riderLat, data.riderLng]);
                    else riderMarker = L.marker([data.riderLat, data.riderLng], {
                        icon: L.divIcon({ html: '🛵', iconSize: [24,24], className: '' })
                    }).addTo(customerMap).bindPopup('Rider');
                    customerMap.setView([data.riderLat, data.riderLng], 14);
                }
            }
            if ((data.status === 'Delivered' || data.status === 'Rejected') && customerMap) {
